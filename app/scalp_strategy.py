...
from .analyzer import Kline, AnalysisResult, MarketMicrostructure, atr, ema, hma, _get_bias, build_score_breakdown, find_strongest_reaction_level
...
def analyze_scalp_precision(symbol: str, k4h: List[Kline], k1h: List[Kline], k15m: List[Kline],
    ...
    # 🆕 اختيار أقوى نقطة دخول (بطلب صريح، نفس منطق stop_hunt وfabio_scalper):
    ...
    # 🔴 تصحيح إضافي (بطلب صريح، بعد اكتشاف تناقض جوهري لسا موجود): 150 شمعة
    # (12.5 ساعة) لسا بعيدة جداً عن أفق الدخول الفعلي (15 دقيقة) — نمط سكالب
    # يتحرك صح بالاتجاه، لكن يحتاج وقت طويل جداً (ساعات) يوصل هدف مبني على
    # مدى نصف يوم كامل، فيتعرّض لضجيج/انعكاس السوق الطبيعي بمنتصف الطريق قبل
    # ما يوصل. الآن 25 شمعة (~2 ساعة) — لسا أوسع من نافذة الدخول (15 دقيقة)
    # عشان نتفادى ضجيج نافذة ضيقة جداً، لكن متناسقة فعلياً مع "سكالب سريع"
    # (هدف يتحقق خلال ساعة-ساعتين، مو نص يوم).
    recent_high = max(k.high for k in k5m[-25:])
    recent_low = min(k.low for k in k5m[-25:])
    ...
    # 🔴 تخفيض من 1:5 إلى 1:3 (بطلب صريح): كان الحد القديم (5:1) يتعارض مع
    # تصحيح الهدف الواقعي — هدف صادق وقريب (يتناسب مع سكالب سريع فعلي) غالباً
    # ما يحقق 5:1، فيرفض أغلب الصفقات رغم إنها واقعية وقابلة للتحقق سريعاً.
    # 1:3 يبقى معياراً صارماً حقيقياً، بدون ما يلغي الهدف من صحّته لأجل رقم
    # تعسفي أعلى.
    if rr < 3.0:
        _log("❌ فرض عائد/مخاطرة لا يقل عن 1:3 (حقيقي بدون تحايل)", f"الناتج 1:{rr} أقل من المطلوب — رفض", False)
        return None
    _log("✅ عائد/مخاطرة يحقق الحد الأدنى المطلوب (1:3)", f"1:{rr}", True)
