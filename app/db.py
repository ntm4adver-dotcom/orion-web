...
DEFAULT_SETTINGS: Dict[str, Any] = {
    ...
    # 🔴 إصلاح جذري (بطلب صريح، بعد اكتشاف السبب الجذري لخسائر متكررة): كان الافتراضي
    # Market فوري — يعني الصفقة تُنفَّذ **لحظة اكتشاف الإشارة** بسعر السوق الحالي،
    # بينما كل استراتيجية بالتطبيق مبنية على انتظار السعر يرجع لمنطقة دخول أفضل
    # (entry_price) قبل الدخول فعلياً — فيه فحص صريح بكل استراتيجية يرفض الإشارة
    # لو السعر الحالي لسا ما وصل هذي المنطقة! التنفيذ بـMarket كان يتجاهل هذا
    # المنطق بالكامل: يشتري/يبيع فوراً بسعر أسوأ من المخطط، بينما الوقف والهدف
    # يبقوا محسوبين بالنسبة لـentry_price الأصلي — فالمخاطرة الفعلية تكبر والعائد
    # الفعلي يصغر عن كل ما فحصته فلاتر الاستراتيجية (rr>=2 مثلاً يصير أقل فعلياً).
    # الآن الافتراضي Limit عند entry_price بالضبط — التنفيذ الحقيقي يطابق التصميم.
    "is_instant_entry_enabled": 0,              # أمر محدد (Limit) عند سعر الدخول المحسوب — بدل سوق فوري (Market)
