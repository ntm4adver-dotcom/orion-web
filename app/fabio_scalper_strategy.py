...
def analyze_fabio_scalper(symbol: str, k4h, k1h, k15m, k5m, k_daily,
    ...
    # 🔴 تصحيح تناقض جوهري (بطلب صريح): 400 شمعة (~4.17 يوم) بعيدة جداً عن أفق
    # "سكالب" فعلي — بروفايل فوليوم لعدة أيام يعطي مستويات (POC/VAH/VAL) صحيحة
    # إحصائياً، لكن الوصول لها من نقطة دخول سكالب سريعة يتطلب وقت طويل جداً
    # (أيام أحياناً)، بعيد عن روح "سكالب" الحقيقية. الآن 100 شمعة (~25 ساعة ≈
    # يوم تداول واحد تقريباً) — لسا كافية إحصائياً لبروفايل فوليوم ذو معنى،
    # لكن أقرب بكثير لأفق زمني يتناسب فعلياً مع سكالب.
    window = k15m[-100:] if len(k15m) >= 100 else k15m
    if len(window) < 60:
        _log("عدد شموع كافٍ لبناء بروفايل الفوليوم", len(window), False)
        return None

    # 📊 فلتر مبني على بيانات إنتاج فعلية: منهج فابيو فالنتيني الحقيقي مصمم لأسواق
    # عميقة السيولة (عقود مؤسسية/عملات كبرى) — بروفايل الفوليوم وCVD يفقدان معناهما
    # على عملات رقيقة السيولة (يكفي محفظة وحدة كبيرة تشوّه القراءة بالكامل). فحص
    # حقيقي لسجل صفقات فعلي أثبت نسبة نجاح متدنية جداً (~13%) حصراً على عملات ميم/
    # صغيرة، فنرفض التداول عليها بهذي الاستراتيجية تحديداً (باقي الاستراتيجيات غير
    # مقيّدة بهذا الفلتر، لأنها لا تعتمد بنفس القدر على عمق السيولة الحقيقي).
    avg_notional_per_candle = sum(k.volume * k.close for k in window) / len(window)
    _log("متوسط قيمة التداول لكل شمعة 15د (اختبار عمق السيولة)", f"${avg_notional_per_candle:,.0f}")
    if avg_notional_per_candle < 50_000:
        _log("❌ فلتر عمق السيولة الدنيا (منهج فابيو يحتاج سوق عميق حقيقي)",
             f"${avg_notional_per_candle:,.0f} أقل من الحد الأدنى ($50,000) — رفض", False)
        return None
    ...
    if is_imbalance:
        ...
        # 🔴 إصلاح إضافي: كان يستخدم بيانات قليلة بصمت (حتى شمعة أو شمعتين) لو k4h
